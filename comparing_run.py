	...
	def get_stat_value(self, function):
		for clss in range(1, self.NUM_CLASS):
			...
			for sbjct in self.__subjects_list:	
				cm = CardioCascadeNet.CompareMatrix(f'{self.MASKS_DIR}/{sbjct}', f'{self.NEW_UNET3_MASK_PATH}{sbjct}', clss)
				# cm = CardioCascadeNet.CompareBullsEyeMatrix(f'{self.DATASET_DIR}{self.DATASET_NAME}_mask_bullmasks/{sbjct}', f'{self.NEW_UNET5_MASK_PATH}{sbjct}', clss)
