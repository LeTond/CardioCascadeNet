...
class Inference(MetaParameters):
    ...
    def get_origin_images(self, file_name, masks_list, unet_type, mask_type, nifti_type, dicom_type):
        if nifti_type == True:
            ...
            GetListImages(file_name, self.dataset_path, unet_type, mask_type).nifti_list(masks_list)

        elif dicom_type == True:            
            images, templates, image_shp, def_coord = \
            GetListImages(file_name, self.dataset_path, unet_type, mask_type).dicom_array(masks_list)
        ...
        if dicom_type:
            for slc in range(len(file_name)):
                if model_fold == self.UNET3_FOLD or model_fold == self.UNET4_FOLD:
                    DicomSaver(masks_list[:, :, slc], file_name[slc], file_dir).save_dicom_mask()
                else:
                    DicomSaver(masks_list[:, :, slc], file_name[slc], file_dir).save_dicom()
    ...
    def run_process(self):
            ...
            if file_name.endswith('.nii'):
                ...
                if self.UNET4 is True:
                    masks_list_04 = self.model_inference(
                        ...
                        model_fold = self.UNET4_FOLD, unet_type = 'cropp', mask_type = 'myo_level', 
                        pdf_flag = False, nifti_type = True, dicom_type = False)
        ...
        for file_name in dataset_list:
            if file_name.endswith('.dcm'):                
                patient_name = ReadImages(f'{file_name}').get_dcm_name()
        ...
        # mask_type = [None, None, None, 'myo_level', 'infer_bull_level']
            ...
            for key_name in dict_subnames.keys():
                ...
                if self.UNET3 is True:
                    masks_list_03 = self.model_inference(
                        ...
                        model_fold = self.UNET3_FOLD, unet_type = 'close_cropp', mask_type = None, 
                        pdf_flag = True, nifti_type = False, dicom_type = True)
